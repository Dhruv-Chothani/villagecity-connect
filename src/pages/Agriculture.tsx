...
import { CropPrice, FarmingTip, Fertilizer, Machinery } from "@/types";

const Agriculture = () => {
  const { farmingTips, cropPrices, fertilizers, machinery, addSubmission } = useData();
  const { user } = useAuth();
  ...
  return (
    <div className="container py-8">
      <div className="mb-6 flex items-center gap-3">
        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-sector-agriculture text-primary-foreground shadow-lg"><Tractor className="h-6 w-6" /></div>
        <div><h1 className="text-2xl font-bold">Agriculture Sector</h1><p className="text-sm text-muted-foreground">Farming Tips, Seeds & Fertilizers, Machinery</p></div>
      </div>
      <div className="mb-6">
        <h2 className="mb-3 text-lg font-bold">📊 Today's Crop Market Prices</h2>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {cropPrices.map((c: CropPrice) => (
      ...
      <Tabs defaultValue="tips" className="w-full">
        ...
        <TabsContent value="tips">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {farmingTips.filter((t: FarmingTip) => t.title.toLowerCase().includes(search.toLowerCase())).map((t: FarmingTip) => (
              <div key={t.id} className="sector-card">
                <div className="flex items-center gap-2"><span className="rounded-full bg-sector-agriculture/10 px-2 py-0.5 text-xs font-medium text-sector-agriculture">{t.season}</span><p className="text-xs text-muted-foreground">{t.date}</p></div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {fertilizers.map((f: Fertilizer) => (
              <div key={f.id} className="sector-card">
                <h3 className="font-bold">{f.name}</h3><span className="rounded-full bg-muted px-2 py-0.5 text-xs">{f.type}</span>
                <p className="mt-1 text-sm text-muted-foreground">{f.usage}</p>
                <p className="mt-2 text-lg font-bold text-primary">{f.price}</p>
                <Button size="sm" className="mt-3" onClick={() => { addSubmission({ type: "enquiry", sector: "Agriculture", userName: user?.name || "Guest", userEmail: user?.email || "", userPhone: "", details: { item: f.name, price: f.price } }); toast.success("Enquiry submitted!"); }}>Enquire</Button>
              </div>
            ))}
          </div>
        </TabsContent>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {machinery.map((m: Machinery) => (
              <div key={m.id} className="sector-card">
                ...
                <Button size="sm" className="mt-3" disabled={!m.available} onClick={() => { addSubmission({ type: "booking", sector: "Agriculture", userName: user?.name || "Guest", userEmail: user?.email || "", userPhone: "", details: { machine: m.name, type: m.type, price: m.price } }); toast.success("Booking confirmed!"); }}>{m.type === "Rental" ? "Book Now" : "Buy Now"}</Button>
              </div>
            ))}
          </div>
        </TabsContent>
