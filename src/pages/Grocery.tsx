...
import { GroceryItem, GroceryStore, SubscriptionPlan } from "@/types";

interface CartItem { id: string; name: string; price: number; qty: number; }
const CATEGORIES = ["All", "Grains", "Beauty", "Dairy", "Snacks"];

const Grocery = () => {
  const { groceryProducts, groceryStores, subscriptionPlans, addSubmission } = useData();
  const { user } = useAuth();
  ...
  const filtered = groceryProducts.filter((p: GroceryItem) => {
    const matchSearch = p.name.toLowerCase().includes(search.toLowerCase()) || p.category.toLowerCase().includes(search.toLowerCase());
    const matchCat = category === "All" || p.category === category;
    return matchSearch && matchCat;
  });

  const addToCart = (p: GroceryItem) => {
    setCart(prev => {
  ...
  const placeOrder = () => {
    addSubmission({
      ...
      userPhone: "",
      details: { items: cart, total, orderType: "Home Delivery" },
    });
    toast.success("Order placed! Home delivery in 2 hours.");
    setCart([]);
    setShowCart(false);
  };
  ...
  return (
    <div className="container py-8">
      <div className="mb-6 flex items-center gap-3">
        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-sector-grocery text-primary-foreground shadow-lg"><ShoppingCart className="h-6 w-6" /></div>
        <div><h1 className="text-2xl font-bold">Home Grocery Store</h1><p className="text-sm text-muted-foreground">Online Ordering, Home Delivery & Daily Subscriptions</p></div>
      </div>
      <Tabs defaultValue="shop" className="w-full">
        ...
        <TabsContent value="stores">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {groceryStores.map((s: GroceryStore) => (
              <div key={s.id} className="sector-card">
                ...
                <div className="mt-2 flex items-center gap-2"><Star className="h-3.5 w-3.5 fill-secondary text-secondary" /><span className="text-sm font-medium">{s.rating}</span>
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${s.delivery ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>{s.delivery ? "🚚 Home Delivery" : "Pickup Only"}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <TabsContent value="subscription">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {subscriptionPlans.map((s: SubscriptionPlan) => (
              <div key={s.id} className="sector-card">
                <h3 className="font-bold">{s.name}</h3>
                <p className="text-sm text-muted-foreground">{s.items}</p>
                <div className="mt-2 flex items-center gap-2 text-xs"><span className="rounded-full bg-muted px-2 py-1">{s.frequency}</span><span className="rounded-full bg-primary/10 px-2 py-1 font-bold text-primary">{s.price}</span></div>
                <Button size="sm" className="mt-3" onClick={() => { addSubmission({ type: "order", sector: "Grocery", userName: user?.name || "Guest", userEmail: user?.email || "", userPhone: "", details: { subscription: s.name, plan: s.price } }); toast.success("Subscription started!"); }}>Subscribe</Button>
              </div>
            ))}
          </div>
        </TabsContent>
        <TabsContent value="offers">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {groceryProducts.filter((p: GroceryItem) => p.offer).map((p: GroceryItem) => (
