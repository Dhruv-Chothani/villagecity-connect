...
const tradeIcons: Record<string, React.ReactNode> = {
  ...
  "Plumber": <Droplets className="h-4 w-4" />,
  "Carpenter": <Hammer className="h-4 w-4" />,
  "House Painter": <PaintBucket className="h-4 w-4" />,
  "Daily Wage Labour": <HardHat className="h-4 w-4" />,
};
  ...
  return (
    <div className="container py-8">
      <div className="mb-6 flex items-center gap-3">
        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-sector-services text-primary-foreground shadow-lg">
          <Wrench className="h-6 w-6" />
        </div>
        <div>
          <h1 className="text-2xl font-bold">Services Sector</h1>
          <p className="text-sm text-muted-foreground">Electricians, Plumbers, Carpenters & More</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="browse" className="w-full">
        ...
        <TabsContent value="browse">
          <div className="mb-4 flex flex-wrap items-center gap-3">
            <SearchFilter value={search} onChange={setSearch} placeholder="Search services..." />
          </div>
          <div className="mb-4 flex flex-wrap gap-2">
            {TRADES.map(t => (
              <Button key={t} size="sm" variant={trade === t ? "default" : "outline"} onClick={() => setTrade(t)} className="gap-1.5">
                {tradeIcons[t]} {t}
              </Button>
            ))}
          </div>
              ...
              <div key={s.id} className="sector-card">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <span className="inline-block rounded-full bg-sector-services/10 px-2 py-0.5 text-xs font-medium text-sector-services">{s.trade}</span>
                    <p className="mt-1 text-xs text-muted-foreground">{s.experience} experience</p>
                  </div>
                  <div className="text-right">
                    <div className="flex items-center gap-1"><Star className="h-4 w-4 fill-secondary text-secondary" /><span className="text-sm font-medium">{s.rating}</span></div>
                    <span className={`text-xs ${s.available ? "text-primary" : "text-destructive"}`}>{s.available ? "Available" : "Busy"}</span>
                  </div>
                </div>
                ...
                {booking === s.id ? (
                  <div className="mt-3 space-y-2 rounded-lg bg-muted p-3">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => { toast.success("Service booked!"); setBooking(null); }}>Confirm</Button>
        ...
        <TabsContent value="book">
          <div className="mx-auto max-w-lg rounded-xl border bg-card p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <Button className="w-full" onClick={() => toast.success("Service request submitted! We'll contact you soon.")}>Submit Request</Button>
            </div>
          </div>
        </TabsContent>
      </Tabs>
